...
${featured
  .map(
    (book) => `- [${book.title}](https://george-books-radar.vercel.app/books/${book.id})
  - Author: ${book.author}
  - Shelf: ${book.shelf}
...
${rotation.map((entry) => `- ${entry.day}: [${entry.book.title}](https://george-books-radar.vercel.app/books/${entry.book.id})`).join("\n")}
